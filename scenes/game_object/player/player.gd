...
func on_arena_difficulty_increased(difficulty: int) -> void:
    var health_regeneration_upgrade_quality = MetaProgression.get_meta_upgrade_quantity("health_regeneration")
    if health_regeneration_upgrade_quality > 0:
        var is_thirty_second_interval = (difficulty % 6) == 0
        if is_thirty_second_interval:
            health_component.heal(health_regeneration_upgrade_quality)
